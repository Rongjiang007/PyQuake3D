...
class QDsim:
    def __init__(self,elelst,nodelst,fnamePara):
        ...
        #self.Bs=self.Bs.transpose()
        #self.Bd=self.Bd.transpose()
    ...
    def Init_condition(self):
        ...
        self.index_normal=np.where(self.eleVec[:,8]>0)[0]
    ...
    def derivative(self,Tno,Tt,state1):
        ...
        # 计算公式
        dVdtau = 2 * V0 / (a * Tno) * safe_exp(-state1 / a) * safe_cosh(Tt / (a * Tno))
        dVdsigma = -2 * V0 * Tt / (a * Tno**2) * safe_exp(-state1 / a) * safe_cosh(Tt / (a * Tno))
        dVdstate = -2 * V0 / a * safe_exp(-state1 / a) * safe_sinh(Tt / (a * Tno))
        dstatedt = b / dc * (V0 * safe_exp((f0 - state1) / b) - np.abs(slipv))
        ...
        return dstatedt,dsigmadt/1e6,dtau1dt/1e6
    
    def derivative_gpu(self,Tno,Tt,state1):
        Tno=Tno*1e6
        Tt=Tt*1e6
        #try:
        ...
        dc = self.dc_gpu
        f0 = self.f0_gpu
        slipv = self.slipv_gpu

        # 计算公式
        dVdtau = 2 * V0 / (a * Tno) * safe_exp(-state1 / a) * safe_cosh(Tt / (a * Tno))
        dVdsigma = -2 * V0 * Tt / (a * Tno**2) * safe_exp(-state1 / a) * safe_cosh(Tt / (a * Tno))
        dVdstate = -2 * V0 / a * safe_exp(-state1 / a) * safe_sinh(Tt / (a * Tno))
        dstatedt = b / dc * (V0 * safe_exp((f0 - state1) / b) - cp.abs(slipv))


        slipv=slipv-self.Vpl_con
        ...
        dtau1dt=(AdotV+self.shear_loading_gpu-self.mu/(2.0*self.Cs)*(dVdsigma*dsigmadt+dVdstate*dstatedt))/(1.0+self.mu/(2.0*self.Cs)*dVdtau)
        #dtau2dt=(AdotV2+self.shear_loading-self.mu/(2.0*self.Cs)*np.sin(self.rake)*(dVdsigma*dsigmadt+dVdstate*dstatedt))/(1.0+self.mu/(2.0*self.Cs)*dVdtau)
        #dVdt=dVdtao*dtaodt+dVdsigma*dsigmadt+dVdstate*dstatedt
    ...
    def simu_forwardGPU(self,dttry):
        ...
        while running:
            Tno_yhk,Tt_yhk,state_yhk,condition1,condition2,hnew1,hnew2=self.RungeKutte_solve_Dormand_Prince_gpu(h)
        ...
        self.slipv_gpu=(2.0*self.V0)*cp.exp(-self.state_gpu/self.a_gpu)*cp.sinh(self.Tt_gpu/self.Tno_gpu/self.a_gpu)
    ...
    def simu_forward(self,dttry):
        nrjct=0
        #kp=0.08
        #self.AbsTol1=1e-1 #absolute error for state
        #self.AbsTol2=400.0  #absolute error for stress
        ...
        #print('maxTt:',np.max(Tt_yhk),'maxstate:',np.max(state_yhk))
        #self.maxslipvque.append(np.max(self.slipv))
        #self.val_nv=clac_revn_vchange_filter(self.maxslipvque)
    ...
    def RungeKutte_solve_Dormand_Prince_gpu(self,h):
        ...
        dstatedt1,dsigmadt1,dtau1dt1=self.derivative_gpu(self.Tno_gpu,self.Tt_gpu,self.state_gpu)
        Tno=self.Tno_gpu
        Tt1o=self.Tt_gpu
        ...
        dstatedt2,dsigmadt2,dtau1dt2=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        dstatedt3,dsigmadt3,dtau1dt3=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        dstatedt4,dsigmadt4,dtau1dt4=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        dstatedt5,dsigmadt5,dtau1dt5=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        dstatedt6,dsigmadt6,dtau1dt6=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        dstatedt7,dsigmadt7,dtau1dt7=self.derivative_gpu(Tno_yhk,Tt1o_yhk,state_yhk)
        ...
        self.Relerrormax2=cp.maximum(Relerrormax2,Relerrormax2_)+1e-10
        ...
        return Tno_yhk,Tt1o_yhk,state_yhk,condition1,condition2,hnew1,hnew2
    ...
    def RungeKutte_solve_Dormand_Prince(self,h):
        ...
        #errormax2=np.max([np.max(Tno_yhk_err),np.max(Tt1o_yhk_err)])
        errormax2=np.max(Tt1o_yhk_err)

        self.Relerrormax1=np.max(np.abs(state_yhk_err/state_yhk8))
        #Relerrormax2=np.max([np.max(np.abs(Tno_yhk_err/Tno_yhk8)),np.max(np.abs(Tt1o_yhk_err/Tt1o_yhk8))])
        self.Relerrormax2=np.max(np.abs(Tt1o_yhk_err/Tt1o_yhk8))
        print('errormax1,errormax2,relaemax1,relaemax2:',errormax1,errormax2,self.Relerrormax1,self.Relerrormax2)


        if((self.maxslipv0)>1e-5):
            self.RelTol1=3e-4
            self.RelTol2=3e-4
        elif((self.maxslipv0)>1e-6):
            self.RelTol1=1e-4
            self.RelTol2=1e-4
        else:
            self.RelTol1=2e-6
