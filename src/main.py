...
if __name__ == "__main__":
    ...
    if(sim0.useGPU==True):
        ...
        for i in range(totaloutputsteps):
            ...
            f.write('%d %f %f %.16e %f %f %f %f\n' %(i,dttry,np.max(np.abs(sim0.slipv1)),np.max(np.abs(sim0.slipv2)),sim0.time,sim0.time/3600.0/24.0,sim0.Relerrormax1,sim0.Relerrormax2))
